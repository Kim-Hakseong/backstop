...
- 재생 진행: 왼→오른쪽 채움, `linear`. 이징 금지. **실제 처리 속도를 그대로 반영한다.** 가짜 애니메이션 금지 — 실측 재생은 1.7ms이므로 화면도 즉시 끝나야 한다. 느리게 보이고 싶으면 그건 재생이 아니라 렌더링이며, 그렇게 라벨링한다
...
### 상태 표시 (재생 중)
타임라인 위에 1px `--info` 세로선이 왼→오른쪽으로 이동. 좌하단에 mono 12px로 진행 카운터 `847 / 1,094`. 스피너 금지 — 숫자가 진행 표시다.

⚠️ **실측 재생 시간은 1.7ms다.** 아래 모션 규칙에 따라 이 진행 표시는 사실상 한 프레임에 끝난다. P4에서 결정할 것: (a) 즉시 완료를 그대로 보여주고 숫자로 말한다 (b) 재생을 스텝 단위로 화면에 흘려보내되 **재생이 아니라 렌더링 속도임을 화면에 명시**한다. 가짜로 느리게 만들지는 않는다.
