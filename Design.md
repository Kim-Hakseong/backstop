...
- **기본값은 다크다.** 시스템 설정(`prefers-color-scheme`)을 따르지 않는다 — 이 제품의 얼굴이
  어두운 계기판이고, 심사위원 기기 설정에 따라 첫 화면이 달라지면 영상과 실물이 어긋난다.
- `?theme=light` / `?theme=dark`로 고정할 수 있다. 촬영과 데모에서 시스템 설정에 흔들리지 않게.
