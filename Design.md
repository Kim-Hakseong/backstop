...
**규칙**
- **장식용 그라데이션 금지.** 단, 카드·배너에 한해 **의미색 블룸 1개**는 허용한다 —
  낮은 알파(0.2 내외)의 radial-gradient로 그 카드의 판정 색(`--block`/`--warn`/`--info`/`--pass`)을
  빛으로 깔아준다. 이건 장식이 아니라 **좌측 3px 색 막대를 대체하는 상태 표시**다.
  막대 방식은 기성 컴포넌트처럼 읽혔고, 같은 사각형이 반복될수록 그게 심해졌다.
- 블룸은 카드당 1개, 한 방향(좌상단)에서만. 두 개 이상 겹치면 그때부터 장식이다.
- 그림자는 카드 1단계만(`0 1px 0 rgba(255,255,255,0.03) inset`) 또는 상단 1px 하이라이트.
