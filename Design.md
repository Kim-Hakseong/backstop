...
### 썸네일 크롭 (라이트 기준 — 08-15 확정)
Devpost 썸네일(1200×630)은 **라이트 테마**의 타임라인 + `DEPLOY BLOCKED` 배너만 잘라 쓴다.
`?theme=light` 로 연 화면에서 크롭한다.

- 배경: 파스텔 워시(분홍→노랑→하늘)가 상단에 옅게 깔린 흰 화면
- 주인공: 4주차 진홍→주황 마커 3개 + 배너의 초점 숫자 `3`
- 텍스트는 `DEPLOY BLOCKED` 한 줄과 숫자 `3` 뿐. 로고·사람·아이콘 없음
- **갤러리에서의 차별점이 "어두운 계기판"에서 "채팅 UI가 아닌 계기판"으로 옮겨간다.**
  파스텔 배경은 흔하지만, 그 위에 점 1,094개와 차단 마커가 깔린 화면은 흔하지 않다.

영상도 라이트로 촬영한다(08-15 결정). 촬영 URL에는 `?theme=light` 를 명시적으로 붙인다 —
촬영 기기에 남은 `localStorage` 선택이 테마를 뒤집는 걸 막기 위해서다.
