...
- 예상: 4h / 실소요: 1.3h
- [x] 08-12. `api/static/index.html` 단일 파일. Canvas 1레이어, devicePixelRatio 반영. 이벤트 점 1,094개(r=1.5, `--inert`) + 부작용 점 42개(r=3, `--fg-muted`) + 주 구분선 6개.
- 스크럽(플레이헤드)은 **만들지 않았다** — 08-12 결정에 따라 진행 애니메이션 자체가 없다. 렌더는 1프레임에 끝난다.

### T4.2 분기 마커 + 카드
- 목표: 붉은 마커 3개. 클릭 시 카드(effect id / 원본 시점 / 재생 의도 / 키 불일치 사유 / OTel 링크)
- 완료 조건: 데모 0:30~1:30 장면이 화면에서 재현된다
- 예상: 3h / 실소요: 0.8h
- [x] 08-12. 4주차 붉은 마커 3개(3px 세로 막대 + 상단 삼각형). 클릭 시 히트 테스트(±6px) → 해당 카드로 스크롤 + 테두리 강조.
- 카드: `EFFECT / RUN / REPLAY` 필드 + `<KeyDiff>` 2행 비교(불일치만 `--block` + `← mismatch`) + 최하단 생성 문장(`--fg-dim`, `— generated summary` 캡션).

### T4.3 상단 카운터
- 목표: `LLM calls during replay` / `external calls during replay: 0` / `DEPLOY BLOCKED` 배너
- 완료 조건: 30초 룰 첫 화면 완성
- 예상: 2h / 실소요: 0.5h
- [x] 08-12. 타일 4개: `LLM CALLS DURING REPLAY 0`(green) / `EXTERNAL CALLS 0`(green) / `LEDGER EVENTS 1,094` / `REPLAY TIME 1.7ms`. `DEPLOY BLOCKED` 배너 + `DEPLOY ALLOWED` 변형 둘 다 구현.
- 🔴 **수치 불일치 잡음**: `gate.json`이 재생 시간만 재서 화면에 0.35ms가 떴는데 README·bench는 1.7ms였다. 같은 주장에 숫자가 둘이면 어느 쪽도 못 믿는다(R7). export를 bench와 **같은 정의**(로드+재생+판정, 5회 중 최속)로 맞췄다. `test_console_export.py`가 export와 즉석 게이트 실행을 대조해 낡은 산출물이 배포되는 걸 막는다.

### T4.4 콘솔 Cloud Run 배포
- 목표: Next.js를 Cloud Run에 배포, 공개 URL 확보
- 완료 조건: URL 접속 시 시드 원장 기반 타임라인이 바로 보인다(로그인 없음)
- 예상: 2h / 실소요: 0.4h
- [x] 08-12. **완료 조건 확인**: https://backstop-api-5nohynuexa-uc.a.run.app/ → 200, 로그인 없음, 헤드리스 브라우저 스크린샷으로 렌더 확인(타일 4개 + 4주차 마커 3개 + `DEPLOY BLOCKED`). revision `backstop-api-00005-m8k`.
- `/gate.json` 200(66KB), `/health` 200 동일 서비스에서 제공.

**P4 검증 게이트**: 공개 URL에서 30초 룰 화면이 로그인 없이 보인다. 태그 `p4-gate-passed`.

✅ **08-12 통과.** 배포 URL에서 30초 룰 화면 확인(스크린샷 검증). 테스트 153건. 태그 `p4-gate-passed`.
계획(08-27) 대비 15일 선행.
...
- 예상: 2h / 실소요: 1.1h
- [x] 08-12. `scripts/make_diagram.py` → `docs/architecture.svg`(99KB, 폰트 base64 임베드) + `docs/architecture.png`(3200×1800, 2x).
- @Design.md §7 규격 준수: 1600×900 / 여백 64 / radius 8 / **직교 라우팅만** / 필수 기술은 **텍스트 배지**(Google 제품 아이콘 미사용 — 브랜드 가이드 위반 회피) / 관문 ①·②는 붉은 세로 막대 / 재생 경로는 점선 + `replay: no egress` / 하단 각주 3줄.
- 필수 기술 3종 라벨 확인: `Google ADK` · `Gemini 3.5 Flash`(2곳) · `Cloud Run`(2곳) · `Pub/Sub` · `Firestore`.
- 🔴 **그리기 순서 버그**: 연결선 라벨을 박스보다 먼저 그려서 박스에 덮였다 — `replay: no egress`가 `replay: no`로 잘려 보였다. 연결선을 **맨 마지막에** 그리도록 바꿨다. 스크린샷으로 확인하지 않았으면 그대로 제출될 뻔했다.
- 각주에 "시뮬레이션 원장" 고지를 넣었다 — 다이어그램만 따로 도는 경우에도 표시되도록.
...
- [~] **초안 완성, 업로드 대기.** `video-out/backstop_demo_gcloud_tts.mp4` — 3분 26초, 1920×1080, 나레이션 포함.
  ...
  - 나머지 38초만 정지 화면(라이브 백엔드 응답 · 아키텍처 다이어그램 · 히어로).
  - 상단에 라이브 URL 캡션 바(편집 오버레이). 헤드리스에 주소창이 없어 브라우저 UI 를 흉내내지 않고 캡션으로 표기했다.
...
- 완료 조건: Rules의 "미공개" 항목(상금 배분 / 심사 배점 / IP)이 채워지거나 여전히 미공개임이 확인된다
- 예상: 0.5h / 실소요: 0.4h
...
| 08-12 | **콘솔을 Next.js 대신 정적 파일 1개로, 기존 FastAPI 서비스에서 서빙** | 화면이 픽스처 위의 단일 뷰다. Node 빌드 + 두 번째 Cloud Run 서비스는 심사위원에게 보이는 이득이 0인데 배포 실패 지점만 늘린다. URL 하나, CORS 없음, 콜드스타트 하나. PRD Section 3의 스택 결정에서 벗어난 지점이라 명시한다 | 콘솔에 상태·라우팅이 필요해지면(현재 계획엔 없음) |
| 08-12 | 콘솔은 `gate.json` 하나만 읽는다 (API 호출 없음) | API가 죽어도, 네트워크가 없어도 화면이 뜬다. 데모 무결성이 부산물이 아니라 요구사항이다(PRD 2-3 Trails 패턴) | 없음 |
| 08-12 | ~~폰트는 시스템 스택~~ → **self-host 확정** (사용자 결정) | 커밋된 `gate.json`과 같은 이유다: 네트워크 없이도 화면이 동일해야 하고, 촬영 중 FOUT이 찍히면 안 된다. Inter는 가변 폰트 1개(48KB)로 400/500/600을 덮고 IBM Plex Mono 2개(20KB) — latin 서브셋 총 76KB. `font-display: block` | 없음 |
...
| 08-15 | P4 | 콘솔 재스타일: 분기 카드를 벤토 그리드 + 의미색 블룸으로 교체(같은 사각형 3개 반복이 기성품처럼 읽혔다). 배너에 초점 숫자 도입 |
| 08-15 | P4 | 2테마 도입(sunset/pastel) + 해·달 토글. 색은 `theme.css` 한 곳, 캔버스도 토큰에서 읽는다. **기본값 라이트, 시스템 설정은 보지 않는다** |
| 08-16 | P5 | 데모 영상 초안 완성(3:26). 브라우저 조작 + 셸 세션 + 라이브 크래시 실녹화, Cloud TTS 나레이션. 업로드만 남음 |
| 08-12 | P4 | **P4 게이트 통과.** T4.1~T4.4 완료. 콘솔 1페이지(Canvas 타임라인 + 마커 카드 + 대조군 토글)를 기존 API 서비스에서 정적 서빙. 배포 URL 렌더 확인. 테스트 153건. 현재 Phase를 P5로 갱신 |
