...
| 08-12 | 폰트는 시스템 스택 (Inter/IBM Plex self-host 안 함) | Design.md는 self-host를 요구하지만 폰트 파일 없이 `ui-monospace`/시스템 산세리프로도 계기판 톤이 유지된다. CDN은 쓰지 않으므로 "CDN 실패로 시스템 폰트가 뜬다"는 원래 리스크는 발생하지 않는다 | 영상에서 톤이 어긋나 보이면 P6에 self-host |
