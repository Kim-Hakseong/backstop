...
- ⚠️ **이 수치가 데모 설계를 바꾼다.** 재생이 1.7ms라 "타임라인이 11초에 걸쳐 채워지는" 0:50–1:10 구간이 성립하지 않는다. Design.md 모션 규칙(가짜 애니메이션 금지)과 정면 충돌하므로 P4에서 결정해야 한다 — @Design.md §3에 선택지 (a)/(b)를 적어 두었다.
