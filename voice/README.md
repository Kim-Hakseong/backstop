...
**2. 배포된 모델에 `lexicon.txt`가 없습니다.**
`pinyin.raw`가 들어있는데 두 번째 컬럼이 로그 확률이라, lexicon 로더가 그걸
발음기호로 읽고 경고를 수만 줄 뱉습니다. → `bootstrap.sh`가 컬럼을 떼고
`lexicon.txt`를 만듭니다.
...
그리고 이 체크포인트는 **중국어/영어** 전용입니다. 한국어 문장을 만들라고 하면
로마자로 바꿔서 넘기긴 하지만, 발음은 근사치입니다. 한국어 출력이 주 목적이면
XTTS-v2나 Chatterbox Multilingual 쪽이 맞습니다.
